...
select * from [dbo].[commandlog_stage_testing_TestCase03] 

/* create temp table for serial result of tempdb */
--drop table #t_results_case3_serial_c1
select 'normal-tempdb' as [TestCase], scenario, row_count, AVG(duration) as avg_duration
into #t_results_case3_serial_c1
from [dbo].[commandlog_stage_testing_TestCase02] as c1
where concurrency = 1 and scenario like 'tempdb%'
group by scenario, row_count
order by scenario, row_count

select 'inmemory-tempdb' as [TestCase], scenario, row_count, AVG(duration) as avg_duration
into #t_results_case3_serial_c2
from [dbo].[commandlog_stage_testing_TestCase03] as c2
where concurrency = 1
group by scenario, row_count
order by scenario, row_count

select *
from #t_results_case3_serial_c1 as c1
--
union all
--
select *
from #t_results_case3_serial_c2 as c2
...
select	--'Serial-AVG' as RunningQuery, 
				--TestCase,
				--row_count,
				cast(row_count as varchar)+ (case TestCase when 'inmemory-tempdb' then ' (M/rOptimizedMD)' else ' (Normal)' end) as [testcase],
...
select 'normal-tempdb' as [TestCase], scenario, row_count, concurrency, duration
into #t_results_case3_parallel_c1
from [dbo].[commandlog_stage_testing_TestCase02] as c1
where concurrency <> 1 and scenario like 'tempdb%'
order by scenario, row_count, concurrency

select 'inmemory-tempdb' as [TestCase], scenario, row_count, concurrency, duration
into #t_results_case3_parallel_c2
from [dbo].[commandlog_stage_testing_TestCase03] as c2
where concurrency <> 1
order by scenario, row_count, concurrency

select *
from #t_results_case3_parallel_c1
union all
select *
from #t_results_case3_parallel_c2
