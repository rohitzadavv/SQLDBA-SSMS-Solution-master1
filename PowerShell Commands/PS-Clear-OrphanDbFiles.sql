...
$InventoryDesco = 'InventoryServer';

# Get list of Server & MountPoint
$tsql_MountPoints = @"
select ServerInstance, MountPoint
from audit_archive..Disk_Consolidation_Mapping as m
...
where IsMoveCompleted = 1
"@
$result_MountPoints = Invoke-DbaQuery -SqlInstance $InventoryDesco -Query $tsql_MountPoints;
...
# Push data of all db files on disk into Infra table audit_archive..OrphanFiles
$Result_ServerDiskFiles | Write-DbaDataTable -SqlInstance $InventoryDesco -Database 'audit_archive' -Table 'OrphanFiles' -AutoCreateTable -Truncate;

# Calculate Space being Freed
$tsql_MountPoints_FreedSpace = @"
select ServerInstance, MountPoint, 
		[Freed-Space-GB] = (SUM(Size_Mb)*1.0)/1024
from audit_archive.dbo.OrphanFiles
where IsOrphan = 1
group by ServerInstance, MountPoint
order by ServerInstance, [Freed-Space-GB] desc
"@
$Result_MountPoints_FreedSpace = Invoke-DbaQuery -SqlInstance $InventoryDesco -Query $tsql_MountPoints_FreedSpace;
...
# Push data of all db files on disk into Infra table audit_archive..OrphanFiles_Renamed
$Result_RenameDbFiles | Write-DbaDataTable -SqlInstance $InventoryDesco -Database 'audit_archive' -Table 'OrphanFiles_Renamed' -AutoCreateTable -Truncate;
